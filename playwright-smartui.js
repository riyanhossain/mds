...
(async () => {
  ...
  console.log("Navigating to MDS...");
  await page.goto("https://mds-weld.vercel.app/");

  console.log("Taking SmartUI screenshot of MDS...");
  ...
  try {
    ...
    // expect(title).toEqual("MDS");
    console.log(
      "SmartUI Test PASSED! Title matched and screenshots validated successfully"
    );
    ...
    console.log('Expected title: "MDS"');
...
const validateSmartUIScreenshots = async (page, screenshotName) => {
  try {
    ...
    if (
      screenshotResponse.screenshotsData &&
      Array.isArray(screenshotResponse.screenshotsData)
    ) {
      ...
      for (let i = 0; i < screenshotResponse.screenshotsData.length; i++) {
        ...
        console.log(
          `Screenshot "${screenshot.screenshotName}": ${screenshot.screenshotStatus}`
        );
        if (screenshot.screenshotStatus !== "Approved") {
          throw new Error(
            `Screenshot status is not approved for the screenshot ${screenshot.screenshotName}`
          );
        }
      }
